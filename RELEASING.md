...
Use the `Release Desktop App` GitHub Actions workflow for public releases. Do
not publish macOS artifacts from the `Desktop Build` workflow; those artifacts
are CI smoke-build outputs and are not guaranteed to be Developer ID signed or
notarized.
...
4. Wait for the macOS verification step to pass. It runs `codesign`, `spctl`,
   and `stapler` before publishing the release.
5. Download the new release DMG from GitHub and open it on a clean macOS
   machine.
